...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`arc-swap`](https://crates.io/crates/arc-swap).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `arc-swap` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Atomic swap of `Arc` references
...
Original work copyrighted by the upstream `arc-swap` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
