# Claude Code Project Instructions — arc-swap-kotlin

## Project Overview

This is **arc-swap-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`arc-swap`](https://crates.io/crates/arc-swap).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/arc-swap/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/arc-swap>
