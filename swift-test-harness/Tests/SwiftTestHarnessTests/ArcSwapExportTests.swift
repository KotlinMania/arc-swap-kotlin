...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `ArcSwap.swiftmodule/`
...
//      `import ArcSwap` above would fail at compile time.
//
//   2. The static archive `libArcSwap.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "ArcSwap" }` and
//      `flattenPackage = "io.github.kotlinmania.arcswap"` configuration in
...
//      via the `ArcSwapLibrary` product.
//
...
final class ArcSwapExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "ArcSwap swift module imported cleanly")
    }
}
